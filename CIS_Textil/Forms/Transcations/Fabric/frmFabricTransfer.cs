...
{
    public partial class frmFabricTransfer : frmTrnsIface
    {
        ...
        private void frmFabricTransfer_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        public void MovetoField()
        {
            try
            {
                ...
                this.txtRefNo.Text = CommonCls.AutoInc(this, "RefNo", "FabricTransID", "");
                ...
                int MaxID = (int)Math.Round(Localization.ParseNativeDouble(DB.GetSnglValue(string.Format(" Select Isnull(Max(FabricTransID),0) From {0}  Where CompID = {1} and YearID = {2}", "tbl_FabricTransferMain", Db_Detials.CompID, Db_Detials.YearID))));

                using (IDataReader reader = DB.GetRS(string.Format("Select * from {0} Where FabricTransID = {1} and CompID={2} and YearID={3}", new object[] { "tbl_FabricTransferMain", MaxID, Db_Detials.CompID, Db_Detials.YearID })))
                {
                    while (reader.Read())
                    {
        ...
        public void FillControls()
        {
            try
            {
                DBValue.Return_DBValue(this, txtCode, "FabricTransID", Enum_Define.ValidationType.Text);
                ...
                DetailGrid_Setup.FillGrid(fgDtls, this.fgDtls.Grid_UID, this.fgDtls.Grid_Tbl, "FabricTransID", txtCode.Text, base.dt_HasDtls_Grd);
                ...
                try
                {
                    ...
                    {
                        if (Localization.ParseNativeInt(DB.GetSnglValue("SELECT count(0) from fn_StockFabricLedger() WHERE RefID<>'' AND RefID='" + CommonLogic.SQuote(fgDtls.Rows[i].Cells["RefID"].Value.ToString()) + "'" + (Db_Detials.CompID.ToString() != "" ? " and CompID=" + Db_Detials.CompID : "") + "")) > 1)
                        {
        ...
        public void SaveRecord()
        {
            try
            {
                ...
                {
                    //strAdjQry = strAdjQry + DBSp.InsertIntoTrasportLedger("(#CodeID#)", ("(#OTHERNO#)"), dtRefDate.Text, Localization.ParseNativeDouble(base.iIDentity.ToString()), Localization.ParseNativeDouble(cboTransport.SelectedValue.ToString()), Localization.ParseNativeDouble(cboDepartFrm.SelectedValue.ToString()), Localization.ParseNativeDouble(cboDepartTo.SelectedValue.ToString()), null, null, null, Localization.ParseNativeDouble(UnitID.ToString()), Localization.ParseNativeInt(string.Format("{0:N0}", CommonCls.GetColSum(this.fgDtls, 16, -1, -1))), Localization.ParseNativeDecimal(string.Format("{0:N2}", CommonCls.GetColSum(this.fgDtls, 17, -1, -1))), Db_Detials.CompID, Db_Detials.YearID, Db_Detials.UserID, DateAndTime.Now.Date);
                }
        ...
        public bool ValidateForm()
        {
            try
            {
                ...
                if (txtRefNo.Text.Trim().Length > 0)
                {
                    ...
                    {
                        strtblName = "tbl_FabricTransferMain";
                        if (Navigate.CheckDuplicate(ref strtblName, "RefNo", txtRefNo.Text, false, "", 0L, string.Format("StoreID = {0} and CompID = {1} and BranchID = {2} and YearID = {3}", Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID), "This Ref No is already used in Entry No : " + DB.GetSnglValue(string.Format("Select EntryNo From {0} Where RefNo = '{1}' and StoreID = {2} and CompID = {3} and BranchID = {4} and YearID = {5} ", new object[] { "tbl_FabricTransferMain", txtRefNo.Text, Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID }))))
                        {
                            txtRefNo.Focus();
                    ...
                    {
                        strtblName = "tbl_FabricTransferMain";
                        if (Navigate.CheckDuplicate(ref strtblName, "RefNo", this.txtRefNo.Text, true, "FabricTransID", Localization.ParseNativeLong(txtCode.Text.Trim()), string.Format("StoreID = {0} and CompID = {1} and BranchID = {2} and YearID = {3}", Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID), "This Ref No is already used in Entry No : " + DB.GetSnglValue(string.Format("Select EntryNo From {0} Where RefNo = '{1}' and StoreID = {2} and CompID = {3} and BranchID = {4} and YearID = {5} ", new object[] { "tbl_FabricTransferMain", txtRefNo.Text, Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID }))))
                        {
                            txtRefNo.Focus();
                            return false;
                        }
                ...
                try
                {
                    if (cboDepartFrm.Text.Trim().ToString() != "-" || cboDepartFrm.SelectedValue != null)
                    {
                        if (dtRefDate.Text != "__/__/____")
                        {
                            ...
                            strQry = string.Format(" Select {0} From {1} ({2}, {3}, {4}, {5}, {6}, {7})", new object[] { strQry_ColName, snglValue, DB.SQuoteNotUnicode(Localization.ToSqlDateString(Conversions.ToString(dtRefDate.Text))), Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID, cboDepartFrm.SelectedValue });
                            #endregion
                        ...
                        if ((txtScan.Text != null) && !string.IsNullOrEmpty(txtScan.Text) && (Localization.ParseNativeInt(cboDepartFrm.SelectedValue.ToString()) > 0))
                        {
                            ...
                            using (IDataReader dr = DB.GetRS(string.Format("{0} {1},{2},{3},{4},{5},{6},'{7}' ", "sp_FetchFabricStock_Barcode", DB.SQuoteNotUnicode(Localization.ToSqlDateString(dtRefDate.Text)), Db_Detials.StoreID, Db_Detials.CompID, Db_Detials.BranchID, Db_Detials.YearID, cboDepartFrm.SelectedValue == null ? "0" : cboDepartFrm.SelectedValue, txtScan.Text.Trim())))
                            {
                                while (dr.Read())
                                {
                                    ...
                                    //_with1.Rows[_with1.RowCount - 1].Cells[4].Value = (dr["FabricID"].ToString() == null ? 0 : (dr["FabricID"].ToString() == "" ? 0 :  Localization.ParseNativeInt(dr["FabricID"].ToString())));
                                    ...
                                    //_with1.Rows[_with1.RowCount - 1].Cells[10].Value = (dr["FabricID"].ToString() == null ? 0 : (dr["FabricID"].ToString() == "" ? 0 : Localization.ParseNativeInt(dr["FabricID"].ToString())));
                                    ...
                                    _with1.Rows[_with1.RowCount - 1].Cells[23].Value = (dr["SubDepartmentID"].ToString());
                    ...
                    if (base.blnFormAction == Enum_Define.ActionType.Edit_Record)
                    {
                        try
                        {
                            ...
                            else
                            {
                                try
                                {
                                    string strQry = string.Format("Update tbl_StockFabricLedger Set IsDeleted=1,DeletedOn=Getdate() Where RowIndex=" + Localization.ParseNativeInt(fgDtls.Rows[fgDtls.CurrentRow.Index].Cells[45].Value.ToString()) + " and UniqueID=" + CommonLogic.SQuote(txtUniqueID.Text) + " and AddedBy=" + Db_Detials.UserID + ";");
                                    DB.ExecuteSQL(strQry);
                                }
                                catch (Exception ex) { Navigate.logError(ex.Message, ex.StackTrace); }
        ...
        private void frmFabricTransfer_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (strUniqueID != null)
            {
                string strQry = string.Format("Delete From tbl_StockFabricLedger Where UniqueID=" + CommonLogic.SQuote(strUniqueID) + " and TransType=" + iIDentity + " and StatusID=1 and AddedBy=" + Db_Detials.UserID + ";");
