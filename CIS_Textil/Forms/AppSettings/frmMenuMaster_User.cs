...
{
    public partial class frmMenuMaster_User : frmMasterIface
    {
        ...
        private void frmMenuMaster_User_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                if (!IsNewReco)
                {
                    if (cboCompany.SelectedValue != null)
                        this.dtEntrys = DB.GetDT(string.Format("Select Distinct MenuID,ParentID,OrderBy,Menu_Caption,Form_Caption,ToolTip,TblName_Main,PmryColumn,SearchQry,SearchQry_Dtls,FormCall,FormCall_Web,FormType,MenuType,VoucherTypeID,RefMenuID,IsForm,IsSeparator,ApplyYear,IsVisible,CompanyID,UserTypeID  From {0} Where MenuID = {1} and CompanyID=" + cboCompany.SelectedValue + " and UserID=" + Db_Detials.UserID + " ", "fn_MenuMaster_User()", this.txtCode.Text), false);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SaveRecord()
        {
            try
            {
                string strMenuMaster_User = string.Empty;
                string strMenuMaster_Comp = string.Empty;
                int QuickAccessMenuID = Localization.ParseNativeInt(DB.GetSnglValue(string.Format("Select MenuID From fn_MenuMaster_Comp() Where Form_Caption='Quick Access'")));
                try
                {
                    strMenuMaster_User += string.Format("Delete from tbl_MenuMaster_User where CompanyID=" + cboCompany.SelectedValue + " and UserTypeID=" + Db_Detials.UserType + " and UserID=" + Db_Detials.UserID + ";");
                    ...
                    {
                        {
                            ...
                            if (strMenuid != "0")
                            {
                                strMenuMaster_User += string.Format("Insert into tbl_MenuMaster_User (MenuID,ParentID,CompanyID,UserTypeID,UserID) values ({0},{1},{2},{3},{4});" + Environment.NewLine,
                                    fgDtls.Rows[i].Cells[0].Value, QuickAccessMenuID,
                                    cboCompany.SelectedValue, Db_Detials.UserType, Db_Detials.UserID);
                ...
                strMenuMaster_User += string.Format(" Update tbl_MenuMaster_User set CompanyID=" + cboCompany.SelectedValue + " where CompanyID is null");
                try
                {
                    DB.ExecuteSQL(strMenuMaster_User + strMenuMaster_Comp);
        ...
        private void addNewRowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    frmStockAdj.Other = Db_Detials.UserType.ToString();
        ...
        private void btnCopy_Click(object sender, EventArgs e)
        {
            ...
            {
                dtcopy = DB.GetDT("Select Distinct MenuID,ParentID,OrderBy,Menu_Caption,Form_Caption,ToolTip,TblName_Main,PmryColumn,SearchQry,SearchQry_Dtls,FormCall,FormCall_Web,FormType,MenuType,IsForm,IsSeparator,ApplyYear,IsVisible from fn_MenuMaster_User() Where CompanyID =" + cboCompany.SelectedValue, false);
        ...
        private void cboCompany_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DB.GetSnglValue("Select * from fn_MenuMaster_User() Where CompanyID =" + cboCompany.SelectedValue) == null || DB.GetSnglValue("Select * from fn_MenuMaster_User() Where CompanyID =" + cboCompany.SelectedValue) == "")
                {
                    EventHandles.CreateDefault_Rows(fgDtls, this.dt_HasDtls_Grd, this.dt_AryCalcvalue, this.dt_AryIsRequired, false, false);
                }
            }
